...
def on_drop(event, values):
    ...
    try:
        ...
        seismic_load = compute_seismic_load (float(site_class_entry.get().strip() or "0"),float (importance_factor_entry.get().strip() or "0"), float (spectral_response_acceleration_entry.get().strip() or "0"))
